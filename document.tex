...
Some geometry conversion tools are provided by pyFAI and documented in \citeasnoun{Detlefs2019}.
...
\caption{The experiment settings tab is used to load the calibration image, set the energy, the calibrant and select the detector for the subsequent analysis.
         The binning mode of the detector is automatically guessed.}
\end{center}
\end{figure}
...
\caption{The cake and integration tab displays the 1D curve and 2D integrated image with the calibrant ring positions overlaid to allow easy validation 
...
the modelization of the translation stage remains hard-coded.
This section explains how to describe programmatically a moving stage in pyFAI, 
...
fitted where the $dist$ (resp. $dist$, $poni_1$ and $poni_2$) depends linearly on the
table position.}
\end{center}
\end{figure}
...
(hypothesis: the translation table is not perfectly parallel to the incident beam).
...
In this example, all motor positions used for calibration were within the accessible range, 
hence positions have been interpolated.
The next example will validate that extrapolation outside the calibration range is also possible.
...
This experiment has been performed using a single module Pilatus (100k)
detector which is mounted on a $2\theta$ arm, moving 
...
diffraction angles ($2\theta >30^o$), to the low curvature of the rings and to the difficulty to 
assign the picked peaks with the proper calibrant ring number.

%The first images presenting two rings correspond to goniometer positions above
%30 degrees. 
Four images
%, taken at goniometer angles of 31.5, 33 and 35 and 35.5 degrees 
have been manually calibrated,
...
fixing the vertical rotation axis of the detector ($rot_1=0$) 
to guarantee some consistency between the calibrations.
...
Finally, all constraints and bounds were removed, and the model has been refined
again and used to generate a \textit{MultiGeometry} object, which is suitable for integrating many
images together.
After integration of all 121 images, the powder diffraction pattern displayed
in Figure \ref{bm20} is obtained (orange curve).
...
The orange curve corresponds to the simple model where $rot_2$ depends linearly on the goniometer angle (6 $DoF$).
The blue curve corresponds to an advanced model where both rotations ($rot_1$ and $rot_2$) 
depend linearly on the goniometer angle (7 $DoF$).}
\end{center}
\end{figure}
...
The fact that both shifts are small suggests to use a first order correction on $rot_1$.
A new model was tested where both the $rot_1$ and $rot_2$ values which are scaling with the 
motor position. 
...
compared to the same pattern recorded with the Pilatus being considered as a point detector. 
The signal/noise ratio was found to be much better with an acquisition time 24 times
faster (10mn instead of 4h). 
The average peak resolution of $FWHM=0.02^o$ obtained on LaB$_6$ (see notebook) 
is a factor 10 worse than the highest achieved resolution with secondary monochromator on insertion devices \cite{Dejoie:kc5085},
but such experiment requires 24 hours acquisition. 
